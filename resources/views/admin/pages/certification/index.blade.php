                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                            <h4 class="mb-sm-0 font-size-18">Certification</h4>
                                                ...
                                                <td>
                                                    @if ($certification->image)
                                                        <img src="{{ asset('uploads/certifications/' . $certification->image) }}"
                                                            alt="{{ $certification->title }}" width="80">
                                                    @else
                                                        No Image
                                                    @endif
                                                </td>
                                                ...
                                                <td>
                                                    @if ($certification->status == 1)
                                                        <a class="btn btn-danger waves-effect btn-circle waves-light"
                                                            href="{{ route('certifications.inactive', $certification->id) }}">
                                                            <i class="fa fa-eye-slash"></i> </a>
                                                    @else
                                                        <a class="btn btn-success waves-effect btn-circle waves-light"
                                                            href="{{ route('certifications.active', $certification->id) }}">
                                                    ...
                                                    <form hidden action="{{ route('certifications.destroy', $certification->id) }}"
                                                        id="form{{ $certification->id }}" method="get">
                                                        @csrf
                                                    </form>
                                                    <button class="btn btn-danger waves-effect btn-circle waves-light"
                                                        onclick="deleteItem({{ $certification->id }});" type="button">
                                                        <i class="fa fa-trash"></i> </button>
                                                </td>
                                                {{-- <td>
                                            <a href="{{ route('certifications.edit', $certification->id) }}" class="btn btn-sm btn-warning">Edit</a>
                                            <form action="{{ route('certifications.destroy', $certification->id) }}" method="POST" style="display: inline-block;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure you want to delete this certification?')">Delete</button>
                                            </form>
                                        </td> --}}
                                            </tr>
                                        @endforeach
